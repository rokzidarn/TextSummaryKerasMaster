...
def convert_single_example(tokenizer, example, max_seq_length):
    words = tokenizer.tokenize(example)
...
train_input_ids, train_input_masks, train_segment_ids = convert_examples_to_features(tokenizer, texts, max_seq_length)

model = build_model(max_seq_length)
initialize_vars(sess)
model.fit([train_input_ids, train_input_masks, train_segment_ids], labels, epochs=10, batch_size=16)
