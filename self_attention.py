...
def seq2seq_architecture(latent_size, embedding_size, vocabulary_size):
    ...
    out, weights = SeqSelfAttention(attention_activation='tanh', name='Attention', return_attention=True)(decoder_outputs)

    decoder_outputs = Dense(vocabulary_size, activation='softmax', name='Final-Output-Dense')(out)
...
def inference(model, latent_size):
    ...
    out, weights = model.get_layer('Attention')(decoder_outputs)
    dense_out = model.get_layer('Final-Output-Dense')(out)
