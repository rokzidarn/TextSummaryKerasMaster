...
def seq2seq_architecture(latent_size, embedding_size, vocabulary_size, batch_size, epochs):
    ...
    _, state_hf, state_hb = tf.keras.layers.Bidirectional(
        tf.keras.layers.GRU(int(latent_size/2), return_state=True, name='Encoder-GRU'))(encoder_embeddings)
    state_h = tf.keras.layers.Concatenate()([state_hf, state_hb])  # Add()
    ...
    seq2seq_model.compile(optimizer=tf.keras.optimizers.RMSprop(),
    ...
    f = open("data/models/bigru_results.txt", "w")
...
def evaluate(encoder_model, decoder_model, titles_train, summaries_train, X_article, word2idx, idx2word, max_length_summary):
    ...
    f = open("data/models/bigru_results.txt", "a")
