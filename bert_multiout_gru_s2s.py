...
seq2seq_model.fit([article_input_ids, article_input_masks, article_segment_ids,
                  ...
                  [numpy.expand_dims(target_input_ids, -1),
                   numpy.expand_dims(target_masks, -1),
                   numpy.expand_dims(target_segment_ids, -1)], epochs=epochs, batch_size=batch_size)

encoder_model, decoder_model = inference(seq2seq_model, latent_size)

for i in range(5):
    inputs = article_input_ids[i], article_input_masks[i], article_segment_ids[i]
