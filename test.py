...
def seq2seq_architecture(latent_size, embedding_size, vocabulary_size):
    ...
    e_out_1, e_state_h_1, e_state_c_1 = LSTM(latent_size, return_sequences=True, return_state=True,
                                         name='Encoder-LSTM-1')(encoder_embeddings)
    e_out_2, e_state_h_2, e_state_c_2 = LSTM(latent_size, return_state=True, name='Encoder-LSTM-2')(e_out_1)
    ...
    d_out_1, d_state_h_1, d_state_c_1 = LSTM(latent_size, return_state=True, return_sequences=True,
                         name='Decoder-LSTM-1')(decoder_embeddings, initial_state=encoder_outputs)
    d_out_2, d_state_h_2, d_state_c_2 = LSTM(latent_size, return_state=True, return_sequences=True,
                                             name='Decoder-LSTM-2')(d_out_1)
    decoder_out = BatchNormalization(name='Decoder-Batch-Normalization-2')(d_out_2)
    decoder_outputs = Dense(vocabulary_size, activation='softmax', name='Final-Output-Dense')(decoder_out)
...
def inference(model):
    ...
    lstm_out_1, state_h_out_1, state_c_out_1 = model.get_layer('Decoder-LSTM-1')(
        [decoder_embeddings, decoder_initial_state_h1, decoder_initial_state_c1])
    lstm_out_2, state_h_out_2, state_c_out_2 = model.get_layer('Decoder-LSTM-2')(lstm_out_1)

    decoder_outputs = model.get_layer('Decoder-Batch-Normalization-2')(lstm_out_2)
    dense_out = model.get_layer('Final-Output-Dense')(decoder_outputs)

    decoder_model = Model([decoder_inputs, decoder_initial_state_h1, decoder_initial_state_c1],
                          [dense_out, state_h_out_2, state_c_out_2])
...
history = seq2seq_model.fit(x=[X_article, X_summary], y=numpy.expand_dims(Y_target, -1), batch_size=batch_size, epochs=epochs)
