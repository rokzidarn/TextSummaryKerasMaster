...
def plot_loss(history_dict):
    ...
    plt.ylabel('Loss')
    plt.legend()
    fig.savefig('data/models/bilstm_train.png')
    ...
    plt.ylabel('Loss')
    plt.legend()
    fig.savefig('data/models/bilstm_valid.png')
...
def seq2seq_architecture(latent_size, vocabulary_size, embedding_matrix, batch_size, epochs, train_article, train_summary, train_target):
    ...
    decoder_lstm = LSTM(latent_size*2, return_state=True, return_sequences=True,
                        dropout=0.2, recurrent_dropout=0.2, name='Decoder-LSTM')
    ...
    inference_state_h_input = Input(shape=(latent_size*2,), name='Hidden-State-Input')
    inference_state_c_input = Input(shape=(latent_size*2,), name='Cell-State-Input')
