...
from tensorflow.python.keras.layers import Input, LSTM, Embedding, Dense, BatchNormalization, TimeDistributed, Activation, dot, concatenate
...
def seq2seq_architecture(latent_size, vocabulary_size, max_len_article, embedding_matrix, batch_size, epochs, train_article, train_summary, train_target):
    ...
    encoder_embeddings = Embedding(vocabulary_size, 300, weights=[embedding_matrix], trainable=False, mask_zero=True,
                                   name='Encoder-Word-Embedding')
    norm_encoder_embeddings = BatchNormalization(name='Encoder-Batch-Normalization')
    encoder_lstm_1 = LSTM(latent_size, name='Encoder-LSTM-1', return_sequences=True, dropout=0.2, recurrent_dropout=0.2)
    ...
    decoder_embeddings = Embedding(vocabulary_size, 300, weights=[embedding_matrix], trainable=False, mask_zero=True,
                                   name='Decoder-Word-Embedding')
    norm_decoder_embeddings = BatchNormalization(name='Decoder-Batch-Normalization-1')
    decoder_lstm_1 = LSTM(latent_size, name='Decoder-LSTM-1', return_sequences=True, dropout=0.2, recurrent_dropout=0.2)
    norm_decoder = BatchNormalization(name='Decoder-Batch-Normalization-2')
    attention_activation = Activation('softmax', name='Attention')
    dense_intermediate = TimeDistributed(Dense(64, activation="tanh", name="Intermediate-Output-Dense"))
    dense_final = TimeDistributed(Dense(vocabulary_size, activation="softmax", name="Final-Output-Dense"))

    d = decoder_embeddings(decoder_inputs)
    d = norm_decoder_embeddings(d)
    decoder_outputs = decoder_lstm_1(d, initial_state=[encoder_last, encoder_last])
    ...
    seq2seq_model = Model(inputs=[encoder_inputs, decoder_inputs], outputs=decoder_last)
...
def inference(encoder_inputs, encoder_outputs, encoder_last):
    encoder_model = Model(inputs=encoder_inputs, outputs=[encoder_outputs, encoder_last])
    ...
    lstm_inf = decoder_lstm_1(embedding_inf, initial_state=[decoder_init_2, decoder_init_2])
    ...
    decoder_model = Model(
        [decoder_inputs] + [decoder_init_2, decoder_init_2],
        [decoder_last_inf] + [lstm_inf])

    return encoder_model, decoder_model


def predict_sequence(seq2seq_model, input_sequence, word2idx, idx2word, max_len):
    decoder_input = np.array([0] * max_len)
    decoder_input[0] = word2idx['<START>']

    for i in range(1, max_len):
        output = seq2seq_model.predict([input_sequence, decoder_input]).argmax(axis=2)
        decoder_input[i] = output[0][0]
        if decoder_input[i] == word2idx['<END>']:
            break

    prediction = [idx2word[x] for x in decoder_input]
    final = [x[0] for x in itertools.groupby(prediction[1:-1])]  # remove <UNK> repetition
    return final


def evaluate(seq2seq_model, max_len, word2idx, idx2word, titles_test, summaries_test, articles_test):
    predictions = []

    for index in range(len(titles_test)):
        input_sequence = articles_test[index:index+1]
        prediction = predict_sequence(seq2seq_model, input_sequence, word2idx, idx2word, max_len)
...
seq2seq_model = seq2seq_architecture(latent_size, vocabulary_size, article_max_len, embedding_matrix, batch_size, epochs,
                                                    train_article, train_summary, train_target)

evaluate(seq2seq_model, summary_max_len, word2idx, idx2word, titles[-test:], summaries[-test:], test_article)
